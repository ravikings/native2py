...
def _document():
    if not GOLDEN_PATH.exists():
        pytest.skip(
            f"no {GOLDEN_PATH.name} recorded yet — run `ngate golden record petro_api`"
        )
...
def test_golden_file_covers_the_api():
    ...
    assert document.get("entries"), (
        ...
        "for why, then re-record with `ngate golden record petro_api`"
    )
...
def test_golden_file_records_the_toolchain_it_was_recorded_on():
    ...
    assert not missing, (
        ...
        "`ngate golden record petro_api` to capture it" % missing
    )
