...
preprocessing step runs before the parser: `native2py/preprocess.py` expands
...
without the package (`parser: regex`, or `$NATIVE2PY_FORTRAN_PARSER`).
...
(`NATIVE2PY_MAX_ARRAY_ITEMS`, default 65536), emitted for any service that
...
| `bindings/generated/` (C++ only), `CMakeLists.txt`, `python/<name>/` (`__init__.py`, `router.py`, `service.py`, `middleware.py`), `tests/`, `native/_expanded/` (Fortran only), `.native2py/ir.json` | generated | `native2py generate <name>` restores all of it byte-for-byte |
| `requirements.lock` | generated | `native2py lock <name>` |
| `infrastructure/kubernetes/<name>.yaml` | generated | `native2py k8s <name>` |
