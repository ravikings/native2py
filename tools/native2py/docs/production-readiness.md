...
**`NATIVE2PY_DEBUG_ERRORS` must stay off in production.** It is off by default
...
- ⚠️ partial — array parameters carry a `MAX_ARRAY_ITEMS` cap
  (`NATIVE2PY_MAX_ARRAY_ITEMS`, default 65536 —
...
| `NATIVE2PY_MAX_ARRAY_ITEMS` | `65536` | Cap on elements in any array argument. Memory guard, **not** a correctness bound — see [API surface](#api-surface) |
| `NATIVE2PY_API_KEYS` | *(unset)* | Comma-separated keys, read at startup. Only consulted when the service was generated with `auth: api_key` |
| `NATIVE2PY_DEBUG_ERRORS` | *(unset — off)* | Adds native exception text to the 500 body. **Leave off in production** |
...
fixes it.** The API native2py generates from this header is session-shaped —
...
tooling design.md §19–22 specifies that native2py still does not generate for
your service.
