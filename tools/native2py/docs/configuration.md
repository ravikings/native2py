...
One file per service, at `services/<name>/native2py.yaml`. Along with your
...
This is the real `services/petro_api/native2py.yaml`, the one service
...
bearing — the C++ parsers fall back to `[[native2py::expose]]` annotations in
...
root, since `COPY` can't reach outside a service directory — `native2py
...
| `parser` | no | `auto`/`clang`/`regex` | ignored — use `$NATIVE2PY_FORTRAN_PARSER` |
| `clang` | no | ✅ | ignored |
...
`-D` defines applied when native2py runs `gfortran -cpp` over preprocessed
...
image layer. The generated middleware reads keys from `NATIVE2PY_API_KEYS` (a
...
|---|---|---|
| `NATIVE2PY_CPP_PARSER` | generator | `auto`/`clang`/`regex`; overrides `parser:` |
| `NATIVE2PY_FORTRAN_PARSER` | generator | `auto`/`fparser2`/`regex` |
| `NATIVE2PY_LIBCLANG` | generator | path to a libclang the loader cannot find itself |
| `NATIVE2PY_API_KEYS` | service | comma-separated valid API keys; required when `api.auth: api_key` |
| `NATIVE2PY_MAX_ARRAY_ITEMS` | service | cap on any array argument's length (default 65536) |
| `NATIVE2PY_MAX_REQUEST_BYTES` | service | request body size cap |
| `NATIVE2PY_RATE_LIMIT_PER_MINUTE` | service | per-client request rate cap |
| `NATIVE2PY_DEBUG_ERRORS` | service | include exception detail in error responses — never set in production |

`NATIVE2PY_MAX_ARRAY_ITEMS` is a **memory guard, not a bounds check**. The IR
