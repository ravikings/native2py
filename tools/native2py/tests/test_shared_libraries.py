...
from native2py.generators import cmake_gen, docker_gen
...
def test_config_roundtrips_libraries(tmp_path):
    ...
    assert yaml.safe_load((tmp_path / "native2py.yaml").read_text())["libraries"] == ["common-cpp"]
...
def test_config_without_libraries_omits_the_key(tmp_path):
    ...
    assert "libraries" not in yaml.safe_load((tmp_path / "native2py.yaml").read_text())
