...
from native2py.config import ClangConfig, ExposeConfig, ServiceConfig
from native2py.parsers import cpp as cpp_parser
from native2py.parsers import cpp_ast, cpp_regex
...
def test_auto_backend_prefers_clang(monkeypatch):
    # $NATIVE2PY_CPP_PARSER outranks "auto", so clear it: this asserts what
    # auto-selection does, not what the environment asked for.
    monkeypatch.delenv("NATIVE2PY_CPP_PARSER", raising=False)
...
def test_env_var_selects_the_backend(tmp_path, monkeypatch):
    monkeypatch.setenv("NATIVE2PY_CPP_PARSER", "regex")
    assert cpp_parser.resolve_backend() == "regex"
    monkeypatch.setenv("NATIVE2PY_CPP_PARSER", "clang")
