...
from native2py.generators import cmake_gen, pybind_gen
from native2py.ir import ClassDef, Method, ModuleIR


def _run_cli(args, cwd, expect_ok=True):
    venv_bin = Path(sys.executable).parent
    result = subprocess.run(
        [str(venv_bin / "native2py"), *args], cwd=cwd, capture_output=True, text=True
    )
    if expect_ok:
...
def _service(tmp_path, name="svc", headers=None, extra_yaml=""):
    ...
    if extra_yaml:
        cfg = tmp_path / "services" / name / "native2py.yaml"
