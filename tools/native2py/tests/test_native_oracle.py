...
1. **Fortran, natively.** A driver program compiled by `gfortran` against
   ...
   decks. No native2py in the loop at all — this is what the library did
...
3. **Python.** The f2py extension, built the way native2py builds it —
...
from native2py.preprocess import expand_includes, resolve_kind_parameters
...
def fortran_objects(workspace: Path) -> list:
    """The F77 decks as ordinary object files — no native2py anywhere."""
...
def fortran_native(workspace: Path, fortran_objects) -> dict:
    ...
    the oracle from the rewritten copy would be comparing native2py's output
    against itself.
    """
    gfortran = _require("gfortran")
...
def python_bindings(workspace: Path):
    """The generated Python path: f2py, built the way native2py builds it."""
    _require_numpy()
    _require("gfortran")
    ...
    # The two source rewrites native2py performs, and nothing else. If either
...
def test_the_two_native_languages_agree_with_each_other(fortran_native, cpp_native):
    ...
    # native2py — and every comparison above is measuring the wrong thing.
...
def test_the_committed_golden_file_matches_the_native_binary(fortran_native):
    ...
    if not GOLDEN_JSON.exists():
        pytest.skip(
            ...
            "run `native2py golden record petro_api` against a built extension"
        )
