...
1. `_NATIVE_LOCK` is a `threading.RLock` in ONE interpreter. It serialises
   ...
   lock; native2py cannot express that for you.
...
from native2py.generators import middleware_gen, python_pkg_gen
from native2py.ir import FunctionDef, ModuleIR, Parameter, SkippedSymbol
...
def array_router(monkeypatch):
    """Exec the array router, with NATIVE2PY_MAX_ARRAY_ITEMS under test control.
    ...
    def build(env=None):
        monkeypatch.delenv("NATIVE2PY_MAX_ARRAY_ITEMS", raising=False)
        if env is not None:
            monkeypatch.setenv("NATIVE2PY_MAX_ARRAY_ITEMS", env)
...
def test_a_native_exception_becomes_a_correlatable_json_500(monkeypatch, tmp_path, caplog):
    ...
    with caplog.at_level(logging.ERROR, logger="native2py.petro"):
    ...
    record = next(
        ...
        if r.name == "native2py.petro" and r.levelno == logging.ERROR
    )
...
def test_the_lock_is_per_interpreter_and_does_not_span_workers():
    ...
    load-balanced to different workers; no lock native2py can emit fixes that.
    """
