"""T12 -- `native2py verify`'s aggregate ordering and per-layer reporting.

Spec: design-verification-layers.md section 2.6 ("`native2py verify <name>`
...
from native2py import cli
from native2py import oracle as oracle_lib
...
def all_layers_pass(monkeypatch):
    ...
    # `verify` only runs the invariants layer at all if native2py.yaml
    # declares something -- patch ServiceConfig.load so it always looks
    # declared, without needing a real native2py.yaml on disk.
