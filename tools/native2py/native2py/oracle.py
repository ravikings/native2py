...
IR_RELATIVE_PATH = Path(".native2py") / "ir.json"
...
def _load(service_dir: Path) -> tuple[ModuleIR, dict]:
    ...
    if not ir_path.exists():
        raise OracleError(f"no {ir_path} — run `native2py generate <name>` first")
    module = module_from_dict(json.loads(ir_path.read_text()))

    golden_path = service_dir / golden.GOLDEN_FILENAME
    if not golden_path.exists():
        raise OracleError(
            f"no {golden_path} — run `native2py golden record <name>` first "
...
def _driver_hash_mismatch_message(recorded_sha: str, fresh_sha: str) -> str:
    return (
        ...
        "native2py generator change invalidates every recorded oracle "
        "file'). Re-recording the file is part of upgrading the tool: run "
        "`native2py oracle record` again once you have reviewed the new "
        "driver output."
    )
