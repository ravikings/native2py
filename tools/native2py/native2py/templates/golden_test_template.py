...
before investigating. Then re-record deliberately with:
    ...
    native2py golden record NATIVE2PY_SERVICE_NAME
...
import NATIVE2PY_PACKAGE

GOLDEN_PATH = Path(__file__).resolve().parent.parent / "NATIVE2PY_GOLDEN_FILENAME"
...
def _materialise(value):
    if isinstance(value, dict) and "__struct__" in value:
        native = getattr(NATIVE2PY_PACKAGE, value["__struct__"])()
...
def _invoke(entry):
    ...
    if entry["kind"] == "function":
        target = getattr(NATIVE2PY_PACKAGE, entry["name"])
    elif entry["kind"] == "static":
        target = getattr(getattr(NATIVE2PY_PACKAGE, entry["class"]), entry["name"])
    else:
        cls = getattr(NATIVE2PY_PACKAGE, entry["class"])
...
def _document():
    if not GOLDEN_PATH.exists():
        pytest.skip(
            f"no {GOLDEN_PATH.name} recorded yet — run `native2py golden record NATIVE2PY_SERVICE_NAME`"
        )
...
def test_golden_file_covers_the_api():
    ...
    assert document.get("entries"), (
        ...
        "for why, then re-record with `native2py golden record NATIVE2PY_SERVICE_NAME`"
    )
...
def test_golden_file_records_the_toolchain_it_was_recorded_on():
    ...
    assert not missing, (
        ...
        "`native2py golden record NATIVE2PY_SERVICE_NAME` to capture it" % missing
    )
