...
Requires the `libclang` Python package (``pip install "native2py[clang]"``).
...
# Import failure must not be fatal at import time: native2py's Fortran half,
...
# bring its own. NATIVE2PY_LIBCLANG overrides all of it.
...
def _load() -> bool:
    ...
    override = os.environ.get("NATIVE2PY_LIBCLANG")
    ...
    if override and not Path(override).exists():
        _LOAD_ERROR = f"NATIVE2PY_LIBCLANG={override} does not exist"
    return False
...
class ClangOptions:
    """Compiler flags for the parse. Mirrors native2py.yaml's `clang:` block."""
...
def _unparsed_reason(diagnostic: str) -> str:
    return (
        ...
        "its directory to `clang.include_paths` in native2py.yaml), or a name that "
...
def _diagnostics(tu) -> list[str]:
    ...
    A header that doesn't compile still yields a partial AST, and native2py
...
def parse_header(
    ...
    native2py.yaml's `expose:` block. `extra_known_records` names types defined
            ...
            if expose.is_exposed(cursor.spelling):
                module.skipped.append(
                    SkippedSymbol(
                        ...
                        "need, or exclude it in native2py.yaml.",
                    )
                )
            return
