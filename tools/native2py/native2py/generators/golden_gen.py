...
TEMPLATE = '''# Generated by native2py. Do not edit by hand — re-run `native2py generate`.
...
before investigating. Then re-record deliberately with:
    ...
    native2py golden record {service}
...
GOLDEN_PATH = Path(__file__).resolve().parent.parent / "{golden_filename}"
...
def _document():
    if not GOLDEN_PATH.exists():
        pytest.skip(
            f"no {{GOLDEN_PATH.name}} recorded yet — run `native2py golden record {service}`"
        )
...
def test_golden_file_covers_the_api():
    ...
    assert document.get("entries"), (
        ...
        "for why, then re-record with `native2py golden record {service}`"
    )
...
def test_golden_file_records_the_toolchain_it_was_recorded_on():
    ...
    assert not missing, (
        ...
        "`native2py golden record {service}` to capture it" % missing
    )
'''
