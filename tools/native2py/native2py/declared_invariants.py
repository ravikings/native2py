...
:mod:`native2py.lattice`. `symmetric_in` and `scales_linearly_in` are declared
...
what this module needs: replay `state.setup`'s entries, in `native2py.yaml`
...
def replay_setup(state: StateConfig, golden_document: dict, package) -> None:
    """Replay `state.setup`'s entries, in `native2py.yaml` order.
