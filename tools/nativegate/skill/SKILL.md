...
`ngate` generates the whole native→Python path from one source file:
...
```bash
pip install -e "<repo>/tools/nativegate[clang,build,test]"
```
...
```bash
ngate quickstart path/to/FluidModel.hpp --name fluidmodel --build
```
...
```bash
ngate golden record fluidmodel  # writes services/fluidmodel/golden.json
ngate golden verify fluidmodel  # 4 entry point(s) unchanged (0 not covered).
```
...
ngate build pvt            # wheel via scikit-build-core -> CMake
...
`nativegate.yaml` + `generate`, rather than `quickstart`, which scans the whole
...
nativegate at runtime and builds on a machine that never installed it.

Generated services **do** carry API-key auth (`api: {auth: api_key}` in
`nativegate.yaml`, keys from `NATIVEGATE_API_KEYS`), rate limiting, a request
size cap, request IDs and access logging, an exception handler, `/healthz` and
`/readyz` with SIGTERM draining, and Kubernetes manifests via `ngate k8s`.
...
- **`NATIVEGATE_MAX_ARRAY_ITEMS` is a memory guard, not a bounds check** — the
...
`middleware.py`) — change `nativegate.yaml` and re-run `generate`.
