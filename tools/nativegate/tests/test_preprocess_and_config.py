...
from nativegate import discovery
...
from nativegate.preprocess import (
    EncodingError,
...
def test_marker_lines_default_to_fixed_form(tmp_path):
    ...
    assert "C     --- nativegate: expanded INCLUDE 'GRID.INC' ---" in expanded
    assert "C     --- nativegate: end INCLUDE 'GRID.INC' ---" in expanded
...
def test_free_form_marker_lines_use_bang(tmp_path):
    ...
    marker_lines = [ln for ln in expanded.splitlines() if "nativegate:" in ln]
...
def test_nested_includes_keep_the_chosen_comment_style(tmp_path):
    ...
    assert not any(
        ln.startswith("C") for ln in expanded.splitlines() if "nativegate:" in ln
    )
...
def _write_yaml(tmp_path, data):
    (tmp_path / "nativegate.yaml").write_text(yaml.dump(data, sort_keys=False))
...
def test_expose_false_binds_nothing_rather_than_everything(tmp_path):
    ...
    (tmp_path / "nativegate.yaml").write_text("name: svc\nlanguage: cpp\nexpose: false\n")
