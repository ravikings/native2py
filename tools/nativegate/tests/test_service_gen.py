...
from nativegate.generators import python_pkg_gen, test_gen
...
def test_validate_catches_a_collision_created_by_escaping():
    ...
    """
    from nativegate import ir
    from nativegate.ir import ClassDef, ModuleIR, StructDef
...
def test_cpp_routers_get_no_lock():
    ...
    # equivalent of COMMON that nativegate can see. Locking every C++ service
...
from nativegate.ir import SkippedSymbol
...
def test_an_array_endpoint_declares_a_cap_on_its_length():
    ...
    assert 'os.environ.get("NATIVEGATE_MAX_ARRAY_ITEMS"' in code
...
def test_an_oversized_array_body_is_rejected_with_a_422():
    ...
    code = code.replace(
        'int(os.environ.get("NATIVEGATE_MAX_ARRAY_ITEMS", "65536"))', "4"
    )
    calls = []
