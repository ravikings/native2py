...
`services/<name>/.nativegate/ir.json` is committed, is read by `golden record`
on machines that have neither the headers nor libclang, and outlives the
nativegate that wrote it. That makes serialisation a compatibility surface, not
...
COMMITTED_IR = REPO_ROOT / "services" / "petro_api" / ".nativegate" / "ir.json"
...
def test_newer_minor_loads_but_reports_the_fields_it_dropped():
    ...
    # Everything this nativegate *does* understand still arrived intact.
...
def test_unknown_major_is_refused_naming_both_versions():
    ...
    assert "7.0.1" in message, "the message must name the nativegate that wrote it"
    assert SCHEMA_VERSION in message, "and the schema this nativegate reads"
    assert f"{SCHEMA_VERSION_MAJOR + 1}.0" in message
    assert "ngate generate" in message, "and what to do about it"
...
def test_unknown_major_without_a_writer_version_still_explains_itself():
    ...
    with pytest.raises(IRSchemaError, match="unknown nativegate version"):
        module_from_dict(document)
...
def test_unknown_keys_do_not_pass_silently(mutate, needle):
    ...
    At a version this nativegate fully implements there is no honest reading of
