"""Validation of `state:`/`invariants:`/`ranges:` in nativegate.yaml (T8).
...
def _write_service(tmp_path: Path, extra_yaml: str) -> Path:
    ...
    (service_dir / "nativegate.yaml").write_text(
        "name: svc\n"
        "language: fortran\n"
        "expose:\n"
        "  classes: []\n"
...
def test_load_verification_directly_on_the_spec_example():
    ...
    v = _load_verification(data, Path("nativegate.yaml"), KNOWN)
...
def test_state_block_rejects_unknown_keys():
    ...
    with pytest.raises(ConfigError, match="unrecognised key"):
        _load_verification(data, Path("nativegate.yaml"), KNOWN)
...
def test_petro_api_nativegate_yaml_parses_and_validates_against_real_ir():
    ...
    from nativegate.parsers import fortran
