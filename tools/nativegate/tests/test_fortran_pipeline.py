...
from nativegate.generators import f2py_gen, python_pkg_gen, test_gen
...
def test_extracts_one_routine_from_a_huge_file(tmp_path):
    ...
    the file being huge and full of routines nativegate doesn't understand
    must not matter."""
    noise = "\n\n".join(
        f"""
...
def test_f2py_build_dir_is_pinned_under_nativegate_tree(reservoir_source):
    ...
    # service's own .nativegate/ tree (the same convention as
    # .nativegate/ir.json) makes the build tree persistent and discoverable.
    module = fortran_parser.parse_source(
        reservoir_source, ExposeConfig(functions=["calculate_pressure"])
    )
    cmake = f2py_gen.generate_cmake(module, "reservoir", ["native/pressure.f90"])
    ...
    assert "${CMAKE_CURRENT_SOURCE_DIR}/.nativegate/build" in cmake
    # Before -m, because f2py treats everything after the module name as source.
