...
rules 2-3, 5, 8. See `nativegate/driverbuild.py`'s module docstring for the
build-pipeline gap this module works around (ngate build's f2py/meson
...
from nativegate import buildinfo, driverbuild, wire
from nativegate.cli import main as nativegate_main
from nativegate.drivers.fortran import generate_driver
from nativegate.ir import module_from_dict
...
def petro_extension_build(tmp_path_factory):
    ...
    for why `ngate build` itself does not leave these reachable)."""
...
# --- `ngate build` itself leaves a discoverable compile_commands.json --
...
@requires_gfortran
def test_nativegate_build_leaves_a_discoverable_compile_commands_json(tmp_path):
    ...
    so a real `ngate build <name>` left no discoverable
    `compile_commands.json` or object files behind. `f2py_gen.generate_cmake`
    now pins `--build-dir` under the service's own `.nativegate/` tree, so
    ...
    (root / "services" / "petro_api" / ".nativegate" / "build").mkdir(
    ...
    try:
        ...
        os.chdir(root)
        generate_result = runner.invoke(nativegate_main, ["generate", "petro_api"])
        assert generate_result.exit_code == 0, generate_result.output

        cmake_text = (root / "services" / "petro_api" / "CMakeLists.txt").read_text()
        assert "--build-dir" in cmake_text
        assert "${CMAKE_CURRENT_SOURCE_DIR}/.nativegate/build" in cmake_text

        build_result = runner.invoke(nativegate_main, ["build", "petro_api"])
        assert build_result.exit_code == 0, build_result.output
    finally:
        os.chdir(cwd)

    compile_commands = (
        root / "services" / "petro_api" / ".nativegate" / "build" / "bbdir"
        / "compile_commands.json"
    )
    assert compile_commands.exists(), (
        f"expected {compile_commands} after `ngate build petro_api` — "
...
from nativegate.drivers.cpp import generate_driver as generate_cpp_driver
