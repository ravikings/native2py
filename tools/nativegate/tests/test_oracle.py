"""T5 — the oracle comparator and CLI (`ngate oracle check|show`).
...
from nativegate import driverbuild, golden, oracle, wire
from nativegate.drivers.fortran import generate_driver
from nativegate.ir import module_from_dict
from nativegate.wire import Slot, arg_slot, pack_float, return_slot
...
def _require_petro_fixture():
    if not (PETRO_SERVICE_DIR / "golden.json").exists() or not (
        PETRO_SERVICE_DIR / ".nativegate" / "ir.json"
    ).exists():
...
def test_a_sabotaged_driver_is_classified_as_argument_order_or_transposition():
    ...
    module = module_from_dict(json.loads((PETRO_SERVICE_DIR / ".nativegate" / "ir.json").read_text()))
    ...
    # (nativegate's own local variable naming, taken straight from the
...
def test_build_oracle_document_records_uncovered_entries_as_skipped():
    from nativegate.ir import ModuleIR
...
def _cpp_oracle_service_dir(tmp_path: Path) -> Path:
    from nativegate.ir import FunctionDef, ModuleIR, Parameter, module_to_dict
