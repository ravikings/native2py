"""`ngate serve` and `ngate verify` must fail with instructions.
...
def test_serve_explains_how_to_install_an_unbuilt_service(tmp_path):
    ...
    assert result.exit_code != 0
    assert "ngate build unbuilt_demo" in result.output
...
def test_verify_needs_a_recorded_golden_file(tmp_path):
    ...
    assert result.exit_code != 0
    assert "ngate golden record unbuilt_demo" in result.output


def test_verify_reports_the_golden_layer_by_name(tmp_path):
    """`ngate verify` is the aggregate gate (design-verification-layers.md
