"""The numerical regression harness (nativegate/golden.py).
...
from nativegate.generators import golden_gen
...
def test_generated_golden_test_is_self_contained():
    ...
    # import nativegate would mean shipping the code generator with it, or a
    ...
    assert not [line for line in imports if "nativegate" in line]
...
def test_generated_golden_test_skips_when_nothing_is_recorded(tmp_path):
    ...
    assert "ngate golden record pvt" in code
...
def test_provenance_records_the_toolchain_that_produced_the_numbers():
    ...
    for field in ("platform", "machine", "python", "python_implementation", "nativegate"):
...
def _project_root() -> pathlib.Path:
    return pathlib.Path(nativegate.__file__).resolve().parent.parent
...
def test_rendered_output_compiles():
    ...
    compile(code, "test_golden.py", "exec")
    assert "NATIVEGATE_" not in code, "a placeholder survived substitution"
...
def test_the_generated_test_never_imports_nativegate():
    ...
    for line in code.splitlines():
        stripped = line.strip()
        assert not stripped.startswith(("import nativegate", "from nativegate")), line
...
def test_the_template_is_discovered_as_package_data_by_the_declared_layout():
    ...
    # tooling: `[tool.setuptools.packages.find] include = ["nativegate*"]`
    # only ships the template because `templates` is an importable package.
    from setuptools import find_packages

    found = find_packages(where=str(_project_root()), include=["nativegate*"])

    assert "nativegate.templates" in found
...
def test_the_declared_dependencies_do_not_include_anything_unimported():
    ...
    sources = "\n".join(
        ...
        for path in (root / "nativegate").rglob("*.py")
    )
