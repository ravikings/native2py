...
Why this exists
---------------
nativegate is about to replace a parser front end (ROADMAP 1.4b / 3.2). The only
...
time-dependent is stored, and the writing nativegate's own version is stripped
...
# tests/corpus -> tests -> tools/nativegate -> tools -> repo root
REPO_ROOT = HARNESS_DIR.parents[3]

if __package__ in (None, ""):  # standalone: make `ngate` importable
    sys.path.insert(0, str(HARNESS_DIR.parents[1]))

from nativegate.config import ExposeConfig  # noqa: E402
from nativegate.ir import ModuleIR, module_to_dict  # noqa: E402
from nativegate.parsers.cpp_ast import ClangOptions  # noqa: E402
...
def _front_door(language: str):
    ...
    try:
        return importlib.import_module(f"nativegate.parsers.{language}")
    except Exception:
        return None
...
def _require_front_door(backend: Backend):
    ...
    if module is None:
        raise BackendUnavailable(
            f"nativegate.parsers.{backend.language} will not import, so backend "
...
def snapshot_document(module: ModuleIR, relative_source: Path) -> dict:
    ...
    data.pop("nativegate_version", None)
