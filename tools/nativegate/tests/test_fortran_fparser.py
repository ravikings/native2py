...
def test_fparser2_is_the_default_when_it_is_available(monkeypatch):
    ...
    monkeypatch.delenv("NATIVEGATE_FORTRAN_PARSER", raising=False)
...
def test_the_env_var_selects_a_backend(monkeypatch):
    monkeypatch.setenv("NATIVEGATE_FORTRAN_PARSER", "regex")
    assert fortran_parser.resolve_backend() == "regex"


def test_an_explicit_argument_beats_the_env_var(monkeypatch):
    monkeypatch.setenv("NATIVEGATE_FORTRAN_PARSER", "fparser2")
...
def test_backend_description_says_why_when_it_falls_back(monkeypatch):
    ...
    assert "nativegate[fparser]" in description
...
def test_both_backends_produce_the_same_ir(source):
    ...
    One routine at a time because that is how nativegate is actually driven —
