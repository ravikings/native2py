...
This is **layer 1** of nativegate's three verification layers. It is the
...
`ngate verify <name>` runs all three, in order (oracle, then golden, then
invariants), and reports each one by name.

Everything nativegate touches can move a number without failing a build: a
...
ngate golden record petro_api  # -> services/petro_api/golden.json (commit this)
ngate golden verify petro_api  # -> "10 entry point(s) unchanged (0 not covered)."
```

`golden.json` is the artifact. Commit it, review its diffs like any other
change to behaviour, and let CI run `ngate test petro_api` — the generated
...
```bash
ngate golden record petro_api --force
```
...
```
$ ngate golden verify pvtcheck        # illustrative
...
intended, re-record with `ngate golden record pvtcheck --force`.
```
