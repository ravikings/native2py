...
**nativegate** exposes native C++ and Fortran code to Python and packages it as a
...
```
Existing C++ / Fortran  →  nativegate  →  Python package + FastAPI + Docker
```

You write the native function. nativegate writes the binding code, the CMake
...
- New to nativegate? Start with [Getting started](getting-started.md) — it
...
- Exposing Fortran, especially a large legacy source file? See
  [Exposing Fortran](fortran-guide.md) — this is the case nativegate's
