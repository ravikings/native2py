...
nativegate parses C++ with **Clang** — the same front end that compiles the
...
instead of nativegate guessing them from token shapes.
...
- `#include`d types, macro-generated declarations (the `F77_NAME(...)` bridge
  ...
  via `clang.defines` in `nativegate.yaml`
...
TypeError: scale(): 'data' must be a numpy array of dtype double; nativegate
...
nativegate **will not guess**. You assert it, per function:
...
| `operator+=` and friends | Python's `__iadd__` must return the mutated object, and a C++ `T&` return needs a return-value policy nativegate cannot infer |
...
`pip install "nativegate[clang]"`. This is one of the few places the two C++
...
If a header cannot be parsed — a missing include, usually — nativegate says so
...
Without libclang, nativegate falls back to `cpp_regex.py`, the original
...
```
$ ngate inspect native/Simulator.hpp
parser: clang AST (libclang 18.1.1)
$ ngate inspect --parser regex native/Simulator.hpp
parser: regex reader (selected explicitly)
```

Pick a backend explicitly with `parser: clang | regex | auto` in
`nativegate.yaml`, `--parser` on `inspect`, or `$NATIVEGATE_CPP_PARSER`.
...
shared library cannot be found, point `NATIVEGATE_LIBCLANG` at it.

## Nothing is dropped silently

Every declaration nativegate recognises but cannot bind is reported at
`generate` time with the reason:

```
$ ngate generate sim
  - Simulator::set_permeability_z: 'double*' is a pointer; nativegate cannot
    infer its length. Wrap it in a type that carries its own size
    (std::vector, a span, or a struct) or exclude the symbol in nativegate.yaml.
  - Simulator::add_well: 'WellModel*' refers to 'WellModel', which is only
    ...
    native/ directory, or exclude the symbol in nativegate.yaml.
...
— falling back to any `[[nativegate::expose]]` annotations in the source — but
...
Parameter and return types map deterministically (`nativegate/ir.py::TYPE_MAP`):
...
nativegate skips such symbols instead, naming the actual cause. To bind them,
...
nativegate still only *parses* headers inside `native/`, so classes defined
...
headers declare methods without inline bodies. `ngate build` will succeed
and then fail on first import with an undefined-symbol error.
```

`ngate quickstart` also picks up a same-stem `.cpp` sitting next to the
...
(`tools/nativegate/tests/test_multifile_cpp.py`,
`tools/nativegate/tests/test_shared_libraries.py`) — not just unit-tested
