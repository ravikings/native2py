...
So nativegate expands INCLUDEs itself and hands f2py a flat source file.
    ...
    for line in read_source(source_path, encoding).splitlines():
        ...
        if resolved is None:
            ...
            raise IncludeError(
                ...
                f"Searched: {searched}. Add the directory to `include_paths:` in nativegate.yaml."
            )

        lines.append(f"{marker}--- nativegate: expanded INCLUDE '{include_name}' ---")
        lines.append(
            expand_includes(
                resolved,
                search_paths,
        ...
        lines.append(f"{marker}--- nativegate: end INCLUDE '{include_name}' ---")
...
def run_c_preprocessor(
    ...
    simultaneously live. That is how nativegate used to read these files (with
    ...
    applies the preprocessor the compiler itself would use, so what nativegate
    ...
    if compiler is None:
        raise PreprocessError(
            ...
            "or preprocess the file yourself and point nativegate at the output."
        )
