...
Fortran path:** `ngate build <name>` runs `pip wheel .`, which drives
...
`--build-dir "${CMAKE_CURRENT_SOURCE_DIR}/.nativegate/build"`, so f2py no
...
documented location: `services/<name>/.nativegate/build/bbdir/compile_commands.json`,
with the corresponding `.o` files alongside it. After a real
`ngate build petro_api`, that `compile_commands.json` is directly
...
service's `.nativegate/` tree, so tests don't mutate checked-out fixture
...
def build_extension_with_compile_commands(
    ...
    *,
    ...
    `ngate build` would have performed, had its generated CMake pinned a
...
def build_cxx_extension_with_compile_commands(
    ...
    explicit `-B build_dir`, so — unlike a real `ngate build`'s `pip
