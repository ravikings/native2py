...
def generate_init_py(module: ModuleIR) -> str:
    ...
    if module.language == "fortran" and any(fn.fortran_module for fn in module.functions):
        ...
        rebindings = "\n".join(
            ...
            + (
                _attr(
                    _attr("_native", fn.fortran_module),
                    (fn.cpp_name or fn.name).lower(),
                )
                if fn.fortran_module
                else _attr("_native", (fn.cpp_name or fn.name).lower())
            )
    ...
    if module.language == "fortran":
        ...
        # but legacy sources (and therefore nativegate.yaml) spell routines in
    ...
    lines = ["# Generated by nativegate. Do not edit by hand — re-run `ngate generate`."]
    if not names:
        ...
        lines.append("# Nothing exposed — check `expose:` in nativegate.yaml.")
    elif by_getattr:
...
ARRAY_CAP_COMMENT = [
    ...
    f'MAX_ARRAY_ITEMS = int(os.environ.get("NATIVEGATE_MAX_ARRAY_ITEMS", "{DEFAULT_MAX_ARRAY_ITEMS}"))',
]
...
def _endpoint_for_function(
    ...
    def _as_json(param) -> str:
        if param.is_array:
            return f"{param.name}.tolist()"
        if param.type == "str":
            # f2py hands CHARACTER outputs back as BYTES, blank-padded to the
            # declared length. Bytes are not JSON, and the padding is Fortran
            # bookkeeping, not data.
            return f"_n2p_text({param.name})"
        return param.name
...
def generate_router_py(module: ModuleIR, service_name: str) -> str:
    ...
    * a signature nativegate cannot serialise (a bound *class* as a parameter
    ...
    needs_text_helper = any("_n2p_text(" in line for line in endpoint_lines)
    ...
    if needs_text_helper:
        lines.extend(
            [
                "def _n2p_text(value) -> str:",
                '    """A CHARACTER output as a Python str.',
                "",
                "    f2py returns bytes, blank-padded to the declared length —",
                "    Fortran bookkeeping, not data. Latin-1 because it cannot",
                "    fail: every byte maps, so a deck that stuffed binary into",
                "    a CHARACTER still round-trips instead of raising mid-request.",
                '    """',
...
def _unexposed_route(module: ModuleIR, names: "_EndpointNames") -> list[str]:
    ...
    404 means "this service was generated before nativegate could tell you".
    """
    ...
    lines.append("def _nativegate_unexposed() -> dict[str, str]:")
