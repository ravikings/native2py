...
_ORDER_INDEPENDENT_HINT = (
    ...
    "`state.mutating` in nativegate.yaml (in review) -- not changing this "
    "check."
)
...
def build_lattices(
    ...
    *that* is golden's/the IR's job, not this module's; it simply is not
    ...
    declared in `nativegate.yaml`) is consulted per entry: a function's
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            result = subprocess.run(
                [python_executable, "-m", "nativegate.structural_invariants", "worker", str(spec_path)],
                capture_output=True,
                text=True,
                env=env,
                timeout=timeout,
...
def _worker_main(argv: list[str]) -> int:
    if len(argv) < 2 or argv[0] != "worker":
        sys.stderr.write(
            "usage: python -m nativegate.structural_invariants worker <spec.json>\n"
        )
        return 2
