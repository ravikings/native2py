...
def map_type(native_type: str) -> str:
    ...
    if normalized not in TYPE_MAP:
        raise NativeTypeError(
            ...
            "or exclude this symbol from nativegate.yaml."
        )
            ...
            if not is_valid_python_name(python_identifier(item.name)):
                problems.append(
                    Problem(
                        ...
                        "identifier; exclude it in nativegate.yaml.",
                    )
                )
...
# --- schema version -----------------------------------------------------
#
# `ir.json` is a *committed artifact*: services/<name>/.nativegate/ir.json is
# checked in, is read by `golden record` on machines that have neither the
# headers nor libclang, and outlives the nativegate that wrote it. Without a
# version stamp, a document written by a future nativegate deserialises into a
...
# nativegate wrote before versioning existed is, by definition, 1.0 minus the
# stamp.
LEGACY_SCHEMA_VERSION = "1.0"

SCHEMA_VERSION_KEY = "schema_version"
WRITER_VERSION_KEY = "nativegate_version"


class IRSchemaError(ValueError):
    """Raised when an ir.json cannot be honestly read by this nativegate."""


class IRSchemaWarning(UserWarning):
    """Warned when an ir.json carries fields this nativegate does not know."""


def nativegate_version() -> str:
    """The installed nativegate version, recorded in every ir.json we write."""
    try:
        from importlib.metadata import PackageNotFoundError, version

        return version("nativegate")
...
def parse_schema_version(raw: object) -> tuple[int, int]:
    ...
    if not match:
        raise IRSchemaError(
            ...
            f'"MAJOR.MINOR" (this nativegate writes "{SCHEMA_VERSION}"). '
            "Regenerate the service with `ngate generate <name>`."
        )
...
def _check_version(data: dict) -> tuple[int, int]:
    ...
    if major != SCHEMA_VERSION_MAJOR:
        ...
        written_by = (
            f"nativegate {writer}" if writer else "an unknown nativegate version"
        )
        ...
        raise IRSchemaError(
            ...
            f"nativegate {nativegate_version()} reads schema {SCHEMA_VERSION} and "
            ...
            + (
                f"Upgrade nativegate (>= {writer}), or regenerate this service "
                "from source with `ngate generate <name>`."
                if major > SCHEMA_VERSION_MAJOR
...
def module_from_dict(data: dict) -> ModuleIR:
    ...
    # later nativegate, so it is reported and skipped rather than fatal.
    ...
    def check_keys(item: dict, allowed: frozenset[str], where: str) -> dict:
        ...
        if strict:
            raise IRSchemaError(
                ...
                "this nativegate fully implements, so these fields are not from a "
                "newer writer. The file is corrupt or hand-edited; regenerate it "
                "with `ngate generate <name>`."
            )
    ...
    def parameters(items) -> list[Parameter]:
        ...
        # older nativegate has none of the fields added since, and **item would
    ...
    if ignored:
        ...
        writer = data.get(WRITER_VERSION_KEY) or "an unknown nativegate version"
        warnings.warn(
            f"This ir.json declares IR schema {major}.{minor} (written by {writer}); "
            f"nativegate {nativegate_version()} implements {SCHEMA_VERSION} and ignored "
            f"{len(ignored)} unknown field(s): {', '.join(sorted(ignored))}. "
            "Upgrade nativegate, or regenerate the service, if any of them matter.",
            IRSchemaWarning,
            stacklevel=2,
        )
    return module
