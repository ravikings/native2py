...
before investigating. Then re-record deliberately with:
    ...
    ngate golden record NATIVEGATE_SERVICE_NAME
...
import NATIVEGATE_PACKAGE

GOLDEN_PATH = Path(__file__).resolve().parent.parent / "NATIVEGATE_GOLDEN_FILENAME"
...
def _materialise(value):
    if isinstance(value, dict) and "__struct__" in value:
        native = getattr(NATIVEGATE_PACKAGE, value["__struct__"])()
...
def _invoke(entry):
    ...
    if entry["kind"] == "function":
        target = getattr(NATIVEGATE_PACKAGE, entry["name"])
    elif entry["kind"] == "static":
        target = getattr(getattr(NATIVEGATE_PACKAGE, entry["class"]), entry["name"])
    else:
        cls = getattr(NATIVEGATE_PACKAGE, entry["class"])
...
def _document():
    if not GOLDEN_PATH.exists():
        pytest.skip(
            f"no {GOLDEN_PATH.name} recorded yet — run `ngate golden record NATIVEGATE_SERVICE_NAME`"
        )
...
def test_golden_file_covers_the_api():
    ...
    assert document.get("entries"), (
        ...
        "for why, then re-record with `ngate golden record NATIVEGATE_SERVICE_NAME`"
    )
...
def test_golden_file_records_the_toolchain_it_was_recorded_on():
    ...
    assert not missing, (
        ...
        "`ngate golden record NATIVEGATE_SERVICE_NAME` to capture it" % missing
    )
