...
nativegate binds (classes, structs, free functions, `extern "C"` blocks) as
...
`ngate inspect --parser regex` selects it explicitly; otherwise see
...
_EXPOSE_ATTR = "[[nativegate::expose]]"
...
def _strip_preprocessor(source: str) -> str:
    ...
    nativegate does not run a preprocessor, so a declaration whose name comes
...
# Pointee spellings that map to a scalar nativegate can bind as a buffer.
...
def _normalize_type(
    ...
    if "vector" in raw_text and "&" in raw_text and not is_return:
        ...
        if _VECTOR_RE.search(before_ref) and "const" not in before_ref.split():
            raise NativeTypeError(
                ...
                "modified vector, or exclude the symbol in nativegate.yaml."
            )
    ...
    if pointer_depth == 1 and base in ("char", "signed char", "unsigned char"):
        ...
        raise NativeTypeError(
            ...
            "input), or exclude the symbol in nativegate.yaml."
        )
    ...
    if vector_match:
        ...
        if inner_is_array:
            raise NativeTypeError(
                ...
                "it, but nativegate has no IR shape for a list of lists, so the "
    ...
    if pointer_depth:
        ...
        if base in _SCALAR_POINTER_ELEMENTS:
            raise PointerLengthUnknown(
                f"'{raw.strip()}' is a pointer; nativegate cannot infer its "
        ...
        raise NativeTypeError(
            f"'{raw.strip()}' is a pointer; nativegate cannot infer its length. "
        ...
        if decl is not None and decl.kind == "operator":
            ...
            module.skipped.append(
                SkippedSymbol(
                    ...
                    '(pip install "nativegate[clang]") to bind it, or expose a '
...
def parse_header(
    ...
    nativegate.yaml's `expose:` block (config takes precedence for legacy code
                ...
                if name:
                    module.skipped.append(
                        SkippedSymbol(
                            ...
                            "macro, nativegate does not run a preprocessor — declare the "
