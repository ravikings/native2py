...
2. `$NATIVEGATE_CPP_PARSER` (`auto` | `clang` | `regex`),
...
_ENV_VAR = "NATIVEGATE_CPP_PARSER"
    ...
    if choice == "clang":
        if not cpp_ast.is_available():
            raise ParserUnavailable(
                ...
                '`pip install "nativegate[clang]"`, point NATIVEGATE_LIBCLANG at a '
                "libclang shared library, or set parser: regex in nativegate.yaml."
            )
        return "clang"
